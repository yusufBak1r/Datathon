    ...
    feature_cols = ["session_length", "n_events", "n_unique_products",
                    ...
                    "avg_event_hour", "most_common_day", "is_weekend_session"]
    ...
    # ** Model Optimizasyonu Başlangıcı **
    # Tanımlayacağınız hiperparametre kombinasyonlarını belirleyin.
    # Bu parametreler modelin performansını doğrudan etkiler.
    param_grid = {
    ...
    # scoring='neg_root_mean_squared_error': RMSE'yi minimize etmeye çalışır
    grid_search = GridSearchCV(estimator=rf, param_grid=param_grid, cv=5, scoring='neg_root_mean_squared_error', n_jobs=-1, verbose=1)

    # Grid Search'ü eğitin
    grid_search.fit(X, y)

    # En iyi modeli ve en iyi hiperparametreleri al
    best_model = grid_search.best_estimator_
    best_params = grid_search.best_params_
    best_score = -grid_search.best_score_ # RMSE'ye çevir

    print("Grid Search Sonuçları:")
    print("En İyi Hiperparametreler:", best_params)
    print("En İyi Çapraz Doğrulama RMSE:", best_score)

    # Final modelini en iyi parametrelerle eğit
    best_model.fit(X_train, y_train)

    # En iyi modelin Validation seti üzerindeki performansı
    y_pred = best_model.predict(X_valid)
    rmse = np.sqrt(mean_squared_error(y_valid, y_pred))
    print("Final Model Validation RMSE:", rmse)
